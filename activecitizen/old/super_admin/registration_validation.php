<?php
	...
	$fname_error = $lname_error = $org_name_error = $org_email_error = $permission_error = $password_error ="";

	$fname = $lname = $org_name = $org_email = $permission = $password = $password1 = "";
	$err_status="Allowed";
	//validation
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(empty($_POST["orgName"])){
			$org_name_error = "Organization name is required";
			$err_status="Not Allowed";
		}else{
		...
		if($err_status!="Not Allowed"){
	      ...
	      $email = mysqli_real_escape_string($con, $email);
			...
			$query2 = "INSERT INTO users (fname, lname, organization, email, password, permission) VALUES ('$fname', '$lname', '$org_name', '$org_email', '$password', '$permission')";
